...
void BinaryOutput::writeCustomBinaryFile(const char* filename, short vertForm, int numVerts, int numIndices, glm::vec3* verts, glm::vec3* color, glm::vec2* texture, glm::vec3* normals, glm::vec3* tangents, glm::vec3* bitangents, GLuint* indices)
{
	int totalBytes = 0;
	...
	case PositionTextureNormalTangentBiTangent :
		vertexSize = sizeof(glm::vec3) * 4 + sizeof(glm::vec2);
		break;
	default :
	...
	totalBytes += writeVertexData(outputStream, vertForm, numVerts, verts, color, texture, normals, tangents, bitangents);
...
int BinaryOutput::writeVertexData(std::ofstream& out, short vertForm, int numVerts, glm::vec3* verts, glm::vec3* color, glm::vec2* texture, glm::vec3* normals, glm::vec3* tangents, glm::vec3* bitangents)
{
	int totalBytes = 0;
	...
	case PositionTextureNormalTangentBiTangent:
		for (int j = 0; j < numVerts; ++j)
		{
			...
			totalBytes += writeVec2(out, texture[j]);
			totalBytes += writeVec3(out, normals[j]);
			totalBytes += writeVec3(out, tangents[j]);
			totalBytes += writeVec3(out, bitangents[j]);
		}
		break;
	default:
