...
void ObjParser::getBiTangents()
{
	...
	for (int i = 0; i < indCount; i += 3)
	{
		...
		finalBiTans[i].x = f * (-deltaUV2.x * edge1.x + deltaUV1.x * edge2.x);
		finalBiTans[i].y = f * (-deltaUV2.x * edge1.y + deltaUV1.x * edge2.y);
		finalBiTans[i].z = f * (-deltaUV2.x * edge1.z + deltaUV1.x * edge2.z);
