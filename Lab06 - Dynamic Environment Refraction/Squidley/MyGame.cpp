...
GLuint passThroughProgramID, pTShaderProgramID, pCshaderProgramID, pCTShaderProgramID, 
		...
		depthShaderProgramID, pTNShaderProgramID;
...
void MyGame::Draw()
{
	glm::mat4 refractProj = glm::perspective(glm::radians(90.0f), (GLfloat)REFRACT_WIDTH / (GLfloat)REFRACT_HEIGHT, 0.1f, FarPlane);
	
	refractTransforms[0] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	refractTransforms[1] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	refractTransforms[2] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0)));
	refractTransforms[3] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(0.0, -1.0, 0.0), glm::vec3(0.0, 0.0, -1.0)));
	refractTransforms[4] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(0.0, 0.0, 1.0), glm::vec3(0.0, -1.0, 0.0)));
	refractTransforms[5] = (glm::lookAt(stealthPos, stealthPos + glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, -1.0, 0.0)));

	glBindFramebuffer(GL_FRAMEBUFFER, refractMapFBO);
	for (int j = 0; j < 6; ++j)
	{
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + j, refractCubeMap, 0);
		RenderEngine::Draw(&refractProj[0][0], &refractTransforms[j][0][0]);
	}
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	TextureInfo texture;
...
void MyGame::SendDataToOpenGL()
{
	...
	render[0].m_vertexShaderInfo.programID = pTNShaderProgramID;
	render[0].m_vertexShaderInfo.callback = uniformCallbackBetterLighting;
...
void MyGame::glGetViewProjectionUniformLocation(Renderable* randy, float* projection, float* cameraView)
{
	...
	{
		glm::mat4 viewToProjectionMatrix = glm::perspective(glm::radians(camera.FOV), ((float)m_window->width()) / m_window->height(), 0.1f, FarPlane);
